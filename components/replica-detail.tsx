	...
	return (
		<ScrollArea className="h-[600px] w-full pr-4">
			<div className="space-y-6">
					...
					<CardContent className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								...
								<p className="font-medium">
									{replica.created_at
										? format(new Date(replica.created_at), 'dd.MM.yyyy HH:mm')
										: 'Нет данных'}
								</p>
							</div>
							<div className="col-span-2">
								...
								<p className="font-medium text-xs truncate">
									{replica.ownerID || replica.owner_uuid}
								</p>
							</div>
						</div>
					</CardContent>
				...
				{replica.suggestedQuestions &&
					replica.suggestedQuestions.length > 0 && (
						<Card className="border border-dark-500 bg-dark-300 text-white">
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<MessageCircleQuestion className="h-5 w-5 text-gray-300" />
							...
							<CardContent>
								<div className="space-y-2">
									{replica.suggestedQuestions.map((question, index) => (
										<div
											key={index}
											className="px-3 py-2 bg-dark-400 rounded-md"
										>
											{question}
										</div>
									))}
					...
					<CardContent className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
							<div>
								...
								<Badge
									variant={replica.video_enabled ? 'success' : 'destructive'}
								>
							...
							<div>
								...
								<Badge
									variant={replica.voice_enabled ? 'success' : 'destructive'}
								>
							...
							<div>
								...
								<p className="font-medium text-xs truncate">
									{replica.elevenLabsID || 'Не указан'}
								</p>
							</div>
							<div>
								...
								<div className="bg-dark-400 p-3 rounded-md">
									<div className="grid grid-cols-2 gap-2">
										<div>
											...
											<p className="font-medium">
												{replica.discord_integration.service_name}
											</p>
										</div>
										<div>
											...
											<Badge
												variant={
													replica.discord_integration.is_active
														? 'success'
														: 'destructive'
												}
											>
							...
							<div className="mt-4">
								...
								<div className="bg-dark-400 p-3 rounded-md">
									<div>
										<p className="text-xs text-gray-400">Имя сервиса:</p>
										<p className="font-medium">
											{replica.telegram_integration.service_name}
										</p>
									</div>
								</div>
							</div>
