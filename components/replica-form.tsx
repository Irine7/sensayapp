...
export default function ReplicaForm({
	onSubmit,
	initialData,
	onCancel,
	isLoading,
	...
	const defaultLlm = {
		...
		systemMessage:
			initialData?.system_message || 'Descriptive, energetic, friendly',
	...
	const [formData, setFormData] = useState<ReplicaCreateUpdateData>({
		...
		shortDescription:
			initialData?.shortDescription || initialData?.short_description || '',
		...
		profileImage:
			...
			'https://placehold.co/400x400/4F46E5/FFFFFF?text=AI+Replica',
		...
		llm: initialData?.llm
			...
			: defaultLlm,
		voicePreviewText:
			"Hi, I'm your Sensay replica! How can I assist you today?",
	});
	...
	useEffect(() => {
		if (formData.name.trim() && (!formData.slug || !formData.slug.trim())) {
		...
		setFormData((prev) => {
			...
			if (name === 'name' && value.trim()) {
				...
				return {
					...prev,
					[name]: value,
					slug: prev.slug || generatedSlug, // Only set if slug is empty
				};
			}
	...
	const handleRemoveEmail = (index: number) => {
		setFormData((prev) => ({
			...prev,
			whitelistEmails:
				prev.whitelistEmails?.filter((_, i) => i !== index) || [],
		}));
	};
	...
	const handleRemoveQuestion = (index: number) => {
		setFormData((prev) => ({
			...prev,
			suggestedQuestions:
				prev.suggestedQuestions?.filter((_, i) => i !== index) || [],
		}));
	};
	...
	const validateForm = (): { valid: boolean; error?: string } => {
		...
		if (formData.shortDescription.length > 50) {
			return {
				valid: false,
				error: 'Short description must be 50 characters or less',
			};
		}
		...
		if (!formData.slug || !formData.slug.trim()) {
			return {
				valid: false,
				error: 'Slug is required. It will be auto-generated from the name.',
			};
		}
		...
		try {
			...
			setFormData((prev) => {
				...
				const updatedLlm = {
					...
					systemMessage:
						data.systemMessage ||
						prev.llm?.systemMessage ||
						'Descriptive, energetic, friendly',
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Textarea
										...
										onChange={handleChange}
										placeholder="John Smith is a 55 year old accountant from Brooklyn."
										required
										maxLength={50}
										className="min-h-[80px] border-dark-500 bg-dark-400 text-white"
									/>
								...
								<div className="flex flex-wrap gap-2 mt-2">
									{formData.tags?.map((tag, index) => (
										<Badge
											key={index}
											variant="outline"
											className="bg-dark-400 flex items-center gap-1"
										>
											{tag}
											<button
												type="button"
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									{formData.whitelistEmails?.map((email, index) => (
										<div
											key={index}
											className="flex items-center justify-between bg-dark-400 p-2 rounded"
										>
											{email}
											<button
												type="button"
