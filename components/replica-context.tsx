...
interface ReplicaContextType {
	...
	selectReplicaByType: (type: string) => void;
}
...
export function ReplicaProvider({ children }: { children: React.ReactNode }) {
	...
	const selectedReplica = replicas.find(
		(replica) => replica.uuid === selectedReplicaUuid
	);
	...
	const refreshReplicas = async () => {
		try {
			...
			const savedUuid =
				selectedReplicaUuid || localStorage.getItem(LOCAL_STORAGE_KEY);

			// Проверяем, существует ли сохраненная реплика в полученном списке
			const replicaExists = replicasList.some(
				(replica) => replica.uuid === savedUuid
			);
			...
			if (replicasList.length > 0) {
				if (replicaExists && savedUuid) {
					...
					console.log(
						'Saved replica not found or not selected, using first available replica'
					);
	...
	// Function to select replica by type (name)
	const selectReplicaByType = (type: string) => {
		const replica = replicas.find(
			(r) =>
				r.name.toLowerCase() === type.toLowerCase() ||
				r.type.toLowerCase() === type.toLowerCase()
		);

		if (replica) {
			setSelectedReplicaUuid(replica.uuid);
			console.log(`Selected replica: ${replica.name} (${replica.uuid})`);
		} else {
			console.warn(`Replica with type "${type}" not found`);
		}
	};
