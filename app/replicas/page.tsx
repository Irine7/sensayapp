...
import {
	SensayReplica,
	ReplicaCreateUpdateData,
} from '@/app/lib/api/sensay-replicas-client';
import {
	fetchReplicas,
	fetchReplicaById,
	createReplica,
...
export default function ReplicasPage() {
	...
	const [selectedReplica, setSelectedReplica] = useState<SensayReplica | null>(
		null
	);
		...
		try {
			...
			const errorMessage =
				error instanceof Error
					? error.message
					: 'Failed to fetch replica details';
			toast({
				title: 'Error',
				description: errorMessage,
				variant: 'destructive',
	...
	const handleOpenEditDialog = async (replicaId: string) => {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : 'Failed to fetch replica';
			toast({
				title: 'Error',
				description: errorMessage,
				variant: 'destructive',
	...
	const handleUpdateReplica = async (data: ReplicaCreateUpdateData) => {
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : 'Failed to update replica';
			toast({
				title: 'Error',
				description: errorMessage,
				variant: 'destructive',
						...
						<TableRow key={replica.uuid}>
							<TableCell>
								<div className="font-medium flex items-center gap-3">
									{/* Add replica image */}
									<div className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-100/10">
										<img
											src={
												replica.profileImage ||
												'https://placehold.co/400x400/4F46E5/FFFFFF?text=AI'
											}
											alt={replica.name}
									...
									<div>
										<div className="flex items-center gap-2">
											{replica.name}
											{replica.private && (
												<Badge variant="outline" className="ml-1 bg-dark-400">
													Private
												</Badge>
											)}
										</div>
										...
										<div className="flex items-center gap-2 mt-1">
											...
											{replica.voice_enabled && (
												<Badge
													variant="outline"
													className="text-xs bg-dark-400"
												>
													Voice
												</Badge>
											)}
											{replica.video_enabled && (
												<Badge
													variant="outline"
													className="text-xs bg-dark-400"
												>
								...
								<div className="text-xs text-muted-foreground mt-1">
									<Popover>
										...
										<PopoverContent className="w-80 p-2 bg-dark-300 border-dark-500 text-white">
											<p className="text-xs">
												{replica.llm?.systemMessage || replica.system_message}
											</p>
										</PopoverContent>
									</Popover>
								</div>
							...
							<TableCell>
								<div className="flex flex-wrap gap-1 max-w-[150px]">
									...
									{replica.tags && replica.tags.length > 3 && (
										<Badge variant="outline" className="text-xs bg-dark-400">
		...
		<div className="container mx-auto py-6 space-y-8">
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Manage replicas for Sensay API
					</p>
				</div>
			...
			<Card className="border border-dark-500 bg-dark-300 text-white">
				<CardHeader>
					...
					<CardDescription>
						List of replicas from Sensay API. These are AI characters that can
					...
					{loading ? (
						<div className="text-center py-8">
							...
							<div className="mt-4 text-muted-foreground">
								Loading replicas from Sensay API...
							</div>
						</div>
					) : (
						replicasTable
				...
				<CardFooter className="flex justify-between">
					<div className="text-sm text-muted-foreground">
						{replicas.length > 0
							? `Found ${replicas.length} replicas`
							: 'No replicas found'}
					</div>
				</CardFooter>
			</Card>
			...
			>
				<AlertDialogContent className="border border-dark-500 bg-dark-300 text-white">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action will delete the replica{' '}
							<strong>{selectedReplica?.name}</strong> and cannot be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
