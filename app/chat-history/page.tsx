...
import type {
	SensayChatHistory,
	SensayChatMessage,
} from '@/app/lib/api/sensay-chat-history';

// Use type from our service
...
export default function ChatHistoryPage() {
	// Define page container styles for better scrolling
	const pageStyles = {
		display: 'flex',
	...
	const [selectedChat, setSelectedChat] =
		useState<ExtendedSensayChatHistory | null>(null);
	...
	const fetchChatHistory = useCallback(async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/sensay/chat-history/debug?replicaUUID=${encodeURIComponent(
					replicaUUID
				)}`
			);
			...
			if (debugData.error) {
				throw new Error(
					`Debug API error: ${debugData.error}. Attempts: ${JSON.stringify(
						debugData.attempts
					)}`
				);
			}
	...
	const handleCreateChatEntry = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/sensay/chat-history?replicaUUID=${encodeURIComponent(
					replicaUUID
				)}`,
				{
					method: 'POST',
			...
			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(
					errorData.error || 'Failed to create chat history entry'
				);
			}
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: 'Failed to create chat history entry. Please try again.',
				variant: 'destructive',
			});
		}
	};
	...
	const handleDeleteChatEntry = async (chatId: string) => {
		...
		try {
			...
			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(
					errorData.error || 'Failed to delete chat history entry'
				);
			}
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: 'Failed to delete chat history entry. Please try again.',
				variant: 'destructive',
			});
		}
	};
	...
	const downloadChatHistory = (chat: ExtendedSensayChatHistory) => {
		...
		const chatIdStr =
			typeof chat.id === 'string'
				...
				? String(chat.id)
				: `chat-${Date.now()}`;
		const exportFileDefaultName = `chat-history-${chatIdStr.substring(
			0,
			8
		)}.json`;
		...
		<div style={pageStyles} className="space-y-6">
			...
			<Card className="border-gray-200 dark:border-gray-800">
				...
				<CardContent className="space-y-4 pt-4">
					<div>
						<div className="flex items-center gap-2 p-2 border rounded-md border-gray-300 bg-gray-50 dark:bg-gray-900/20 dark:border-blue-700">
							...
							{selectedReplica ? (
								...
								<div className="text-gray-500 italic">
									No replica selected. Please select a replica from the header
									dropdown.
								</div>
							)}
						</div>
							...
							<div className="bg-gray-50 dark:bg-gray-900/20 p-4 rounded-md border border-gray-200 dark:border-gray-800">
								...
								<div className="grid grid-cols-2 gap-2 mb-4">
									<div>
										...
										<code className="ml-2 text-xs bg-gray-100 dark:bg-gray-800 p-1 rounded">
											{selectedChat.id}
										</code>
									</div>
									<div>
							...
							<div className="bg-gray-50 dark:bg-gray-900/20 p-4 rounded-md border border-gray-200 dark:border-gray-800">
								...
								<div className="space-y-3">
									{selectedChat.messages.map(
										(message: SensayChatMessage, idx: number) => (
											<div
												key={`msg-${idx}`}
												className={`p-3 rounded-lg ${
													message.role === 'user'
														? 'bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-gray-100 ml-auto max-w-[80%]'
														: 'bg-blue-600 text-white mr-auto max-w-[80%]'
												}`}
											>
												<div className="flex justify-between items-center mb-1">
													<Badge
														variant={
															message.role === 'user' ? 'outline' : 'default'
														}
													>
														{message.role}
													</Badge>
