...
export async function GET(
	...
	try {
		...
		if (!response.ok) {
			...
			return NextResponse.json(
				{
					error: `Failed to fetch replica: ${response.status} ${response.statusText}`,
					details: errorText,
				},
...
export async function PUT(
	...
	try {
		...
		if (!response.ok) {
			...
			return NextResponse.json(
				{
					error: `Failed to update replica: ${response.status} ${response.statusText}`,
					details: errorText,
				},
				{ status: response.status }
			);
...
export async function DELETE(
	...
	try {
		...
		if (!response.ok) {
			...
			return NextResponse.json(
				{
					error: `Failed to delete replica: ${response.status} ${response.statusText}`,
					details: errorText,
				},
				{ status: response.status }
			);
