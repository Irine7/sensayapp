...
export default function TrainingPage() {
			...
			try {
				...
				try {
					...
					toast({
						title: 'Error',
						description:
							'Could not fetch training data. Please check your API settings.',
						variant: 'destructive',
					});
	...
	const handleCreateEntry = async () => {
		if (!content || !replicaUUID) {
			toast({
				...
				description:
					'Please provide a replica UUID and content for the training entry.',
				variant: 'destructive',
			});
			return;
		}
		...
		try {
			...
			if (!response.ok) {
				try {
					...
					throw new Error(
						`Failed to delete training entry: ${response.statusText}`
					);
				}
			}
	...
	const handleFileUpload = async () => {
		...
		try {
			...
			console.log(
				`Начинаем загрузку файла: ${file.name}, размер: ${file.size} байт`
			);

			// Шаг 1: Получаем подписанный URL для загрузки файла
			const getSignedUrlResponse = await fetch(
				`/api/sensay/training/files?filename=${encodeURIComponent(
					file.name
			...
			if (!getSignedUrlResponse.ok) {
				...
				throw new Error(
					errorData.error || 'Failed to get signed URL for file upload'
				);
			}
			...
			if (!signedURL || !knowledgeBaseID) {
				throw new Error(
					'Invalid response from server - missing signedURL or knowledgeBaseID'
				);
			}
			...
			toast({
				title: 'File uploaded',
				description:
					'File has been successfully uploaded and is being processed.',
			});
	...
	return (
		<div
			className="container mx-auto py-8 pb-32"
			...
			<div className="space-y-6">
				...
				<Tabs defaultValue="auto">
					<TabsList className="grid w-full max-w-lg grid-cols-3">
						<TabsTrigger value="auto">Auto-Trainer</TabsTrigger>
